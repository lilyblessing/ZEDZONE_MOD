...
public static class Reflect
{
    ...
    public static bool Set(object obj, string name, object value)
    {
        ...
        if (f != null)
        {
            ...
            catch (Exception e) { Plugin.L.LogError($"[PFridge] SetField {t.Name}.{name} 失败: {e.Message}"); return false; }
        }
        ...
        if (p != null && p.CanWrite)
        {
            ...
            catch (Exception e) { Plugin.L.LogError($"[PFridge] SetProp {t.Name}.{name} 失败: {e.Message}"); return false; }
        }
        ...
        if (setter != null)
        {
            ...
            catch (Exception e) { Plugin.L.LogError($"[PFridge] set_{t.Name}.{name} 失败: {e.Message}"); return false; }
        }

        Plugin.L.LogWarning($"[PFridge] 成员不可写: {t.Name}.{name}");
        return false;
    }
