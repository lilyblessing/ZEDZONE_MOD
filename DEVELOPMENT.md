...
| `ItemAttr` | 物品定义 | `itemId`、`stackNumber`、`weight`、`itemPrice`、`itemType`、`toolType`、**`itemName_Runtime`**（直接中文名）、**`itemDescription_WithLanguage`**、`unlockByDefault`、`recipeDataAry` |
| `RecipeData` | 配方 | `itemId`、`recipeItems`（List<RecipeItemData>）、`outputItemNumber`、`craftPlatform`、`toolType`、`iqReqiared`、`craftReqiared` |
...
```powershell
.\tools\make-release.ps1     # 编译 + 打包 dist\NoteTagPlugin-vX.Y.Z.zip（dll+png+README）
